...
void Chat1::read(){
    ...
    if (collect_reading!=""){
       if (collect_reading[0]=='*' && collect_reading[1]=='*'
            && collect_reading[collect_reading.length()-1]=='*'
            && collect_reading[collect_reading.length()-2]=='*')
       {
        dostavka_otch();
       }
       else {
         communic->append("<b><font color=#008000>От: "+collect_reading+ "</font></p>");
         ////////пакет служебный для определения доставки
    dost_tech="**"+collect_reading+"**";
    socket->writeDatagram(dost_tech.toUtf8(),sending,port);
    ////////
